...
public class Limelight {
  ...
  private NetworkTableEntry cl = lime.getEntry("cl");
  ...
  public void updateLimelightPose() {
    double turretAngle = Math.abs(turret.getAngleRelativeToRobot()) > 360
        ? Math.signum(turret.getAngleRelativeToRobot()) * (Math.abs(turret.getAngleRelativeToRobot() - 360))
        : turret.getAngleRelativeToRobot(); // for reference, this is "on god"
    ...
    Pose2d p = new Pose2d(x, y, r); // TODO dunno if we have to account for the limelight offset in code or just in
                                    // the pipeline. thx brandon for the docs T_T
